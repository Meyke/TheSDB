...
public class SortPlan implements Plan {
	...
	private List<TempTable> doAMergeIteration(List<TempTable> runs) {
		...
		while (runs.size() > 1) {
			TempTable p1 = runs.remove(0); //prendo la prima run
			TempTable p2 = runs.remove(0); //prendo la seconda run
	...
	private TempTable mergeTwoRuns(TempTable p1, TempTable p2) {
		...
		//prendo la tabella (non mi importa i blocchi, perchè guardo records) della run1; 
		Scan src1 = p1.open();
		//prendo la tabella della run 2
		Scan src2 = p2.open();
		//questa sarà la tabella ordinata, o meglio la run, ottenuta dalla fusione delle tabelle precedenti
		TempTable result = new TempTable(sch, tx);
		UpdateScan dest = result.open();

		//vedo se la prima tabella (run) ha ancora un record
		boolean hasmore1 = src1.next();
		//vedo se la seconda tabella (run) ha ancora un record
		boolean hasmore2 = src2.next();
		//se entrambe hanno un record, le confronto
